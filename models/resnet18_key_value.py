...
class center_front(nn.Module):
    def __init__(self, pretrained=False):
        super(center_front, self).__init__()
        model = get_resnet18(pretrained)
        model_children = list(model.children())
        global center_model_length, center_front_model_length
        center_model_length = len(model_children) - num_front_layers - num_back_layers
        center_front_model_length=center_model_length - num_unfrozen_center_layers
       
        
        self.center_front_model = nn.Sequential(*model_children[num_front_layers:center_front_model_length+num_front_layers])
        

        if pretrained:
            
            layer_iterator = iter(self.center_front_model)
          
            for i in range(center_front_model_length):
                layer = layer_iterator.__next__()
                for param in layer.parameters():
                    param.requires_grad = False
    ...
    def forward(self, x):
        x = self.center_front_model(x)
        return x


class center_back(nn.Module):
    def __init__(self, pretrained=False):
        super(center_back, self).__init__()
        model = get_resnet18(pretrained)
        model_children = list(model.children())
        global center_model_length, center_front_model_length, center_back_model_length
        center_model_length = len(model_children) - num_front_layers - num_back_layers
        center_back_model_length=num_unfrozen_center_layers
       
        
        self.center_back_model = nn.Sequential(*model_children[num_front_layers+center_front_model_length:center_model_length+num_front_layers])
        print("length of back part: ", len(list(self.center_back_model)), ":: ", center_back_model_length)

        # if pretrained:
            
        #     layer_iterator = iter(self.center_back_model)
    ...
    def freeze(self, epoch, pretrained=False):
        print("freezing the back part of center model")
       
        num_unfrozen_center_layers=0
        if pretrained:
            
            layer_iterator = iter(self.center_back_model)
            for i in range(center_back_model_length):
                print("freezed")
                layer = layer_iterator.__next__()
                for param in layer.parameters():
                    param.requires_grad = False
...
if __name__ == '__main__':
    ...
    model = center_front(pretrained=True)
    print(f'{model.center_front_model}\n\n')
    model = center_back(pretrained=True)
    print(f'{model.center_back_model}\n\n')
